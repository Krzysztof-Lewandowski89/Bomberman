//licznik bomb
Bomb.count = 0;
//maksymalna liczba bomb do postawienia
Bomb.max_count = 2;
//wybuchy bomb, elementy rysunku
Bomb.elements = {
		...
		for (var i=0; i<4; i++) {
			...
			for (var j=0; j<this.range; j++) {
				...
				if(Game.board.b[ this.tmp_row ][ this.tmp_column ].type!='solid'){
					...
					//
					// Czy animujemy wybuch czy zniszczenie skrzynki
					if(Game.board.b[ this.tmp_row ][ this.tmp_column ].ko_obj){// Jeśli trafiło na coś co po zniszczeniu ma animację (u nas tylko skrzynka, ale może na dalszym etapie dodamy więcej takich elementów)
				...
				//
				//
				// Jeśli trafiło na bombę (syb_type = bomb oraz bum_type nie jest zdefiniowany) odpal bombę
				}else if(Game.board.b[ this.tmp_row ][ this.tmp_column ].sub_type=='bomb' && !Game.board.b[ this.tmp_row ][ this.tmp_column ].bum_type){
					// zmieniam timer bomby, sprawiam, że wybucha
