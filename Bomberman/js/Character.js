...
Character.prototype.draw = function(){
	...
	//
	// rysowanie pomocniczego kwadratu pod postacią (żeby wiedzieć na którym polu stoi)
...
// spr czy bohater wszedł na kurczaka (prawda / fałsz)
Hero.prototype.enemyHitTest = function(){
	// Sprawdzam wszystkich wszystkich wrogów na planszy
	for(var e in Enemy.all){
		// do e przypisuje obiekt a nie tylko jego id
		e = Enemy.all[e];
		// porównuję dokładną pozycję x i y (szerokość kurczaka i bohatera na szerokość pola planszy)
		if((this.row==e.row && e.x+Game.board.fW>this.x && e.x<this.x+Game.board.fW) || (this.column==e.column && e.y+Game.board.fH>this.y && e.y<this.y+Game.board.fH)){
			// Wystarczy, że jedne kurczak dotknie bohatera, żeby zakończyć grę, więc nie trzeba sprawdzać reszty
			return true;
		}
	} 
	// Jeśli nie dotknął żadnego kurczaka
	return false;
}
// rozszerzenie metody draw
Hero.prototype.draw = function(){
	this.parent.draw.call(this);
	// ustawiam KO jeśli bohater został zabity przez kurczaka
